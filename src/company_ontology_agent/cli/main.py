...
from company_ontology_agent.cli.commands_portal import portal_app, validate_bind_host
...
from company_ontology_agent.retrieval.runtime import index_project
...
from company_ontology_agent.workflows.build_graph import build_graph_from_graphify

app = typer.Typer(help="Ontology Atlas enterprise knowledge and impact analysis")
...
def launch(
    ...
    full: bool = typer.Option(
        False,
        "--full",
        help="Force a full Graphify re-extraction instead of an incremental refresh.",
    ),
    allow_network: bool = typer.Option(
        False,
        "--allow-network",
        help="Allow binding outside the local machine; exposes data and cost-bearing RAG APIs.",
    ),
) -> None:
    """Build, publish, index, and open the complete Ontology Atlas workspace."""
    if serve:
        validate_bind_host(host, allow_network=allow_network)
    _launch_project(port=port, host=host, serve=serve, full=full)
    ...
    *, dry_run: bool, export_wiki: bool, prune: PruneMode, full: bool = False
    ...
    if config.graphify.enabled:
        ...
        if graphify_result.exit_code == 0:
            ...
            raise RuntimeError(
                f"Graphify failed (exit={graphify_result.exit_code}); publication was stopped to "
                f"protect the current graph. See {graphify_out / 'GRAPH_REPORT.md'}, fix the "
                "reported extraction problem, and re-run."
            )
    else:
        typer.echo("Graphify disabled.")

    typer.echo("[3/3] Building graph")
    ...
    if export_wiki:
        _export_outputs(root, config, result.graph, dry_run=dry_run)
    return result.graph


def _launch_project(*, port: int, host: str, serve: bool, full: bool) -> None:
    root = find_project_root()
    config = load_project_config(root)
    typer.echo("== Ontology Atlas launch ==")
    portal_url = _portal_url(host, port)
    already_running = False
    if serve:
        port_status, portal_url = _launch_port_status(host, port)
        if port_status == "atlas":
            typer.echo(f"Ontology Atlas is already running at {portal_url}")
            typer.echo("Refreshing its graph, index, and static workspace in place.")
            already_running = True
        if port_status == "occupied":
            typer.echo(
                f"Error: Port {port} is already in use by another application. "
                f"Stop it or run ontology-agent launch --port {port + 1}.",
                err=True,
            )
            raise typer.Exit(code=1)
    graph = _run_pipeline(dry_run=False, export_wiki=False, prune="stale", full=full)
    if config.rag.enabled:
        typer.echo("[4/6] Incrementally indexing retrieval summaries")
        index_project(root)
    else:
        typer.echo("[4/6] GraphRAG disabled; Ask will explain how to enable it")
    typer.echo("[5/6] Building client workspace")
    _export_outputs(root, config, graph, dry_run=False)
    if not serve or already_running:
        typer.echo(f"[6/6] Ready: {root / 'portal' / 'index.html'}")
        return
    try:
        import uvicorn
    except ImportError as exc:
        raise RuntimeError("Launching the portal requires company-ontology-agent[rag].") from exc
    from company_ontology_agent.api.app import create_app

    typer.echo(f"[6/6] Serving Ontology Atlas at {portal_url}")
    uvicorn.run(
        create_app(root, allow_text2cypher=host in {"127.0.0.1", "localhost", "::1", "[::1]"}),
        host=host,
        port=port,
        log_level="warning",
    )
...
def _launch_port_status(host: str, port: int) -> tuple[str, str]:
    """Return whether the requested launch port is free, Atlas, or another service."""
    client_host = _client_host(host)
    portal_url = _portal_url(host, port)
    try:
        connection = socket.create_connection((client_host, port), timeout=0.3)
    except OSError:
        return "free", portal_url
    connection.close()

    openapi_host = f"[{client_host}]" if ":" in client_host else client_host
    try:
        with urlopen(f"http://{openapi_host}:{port}/openapi.json", timeout=1.0) as response:
            payload = json.load(response)
    except (OSError, ValueError):
        return "occupied", portal_url
    info = payload.get("info") if isinstance(payload, dict) else None
    if isinstance(info, dict) and info.get("title") == "Ontology Atlas":
        return "atlas", portal_url
    return "occupied", portal_url
...
def _portal_url(host: str, port: int) -> str:
    client_host = _client_host(host)
    url_host = f"[{client_host}]" if ":" in client_host else client_host
    return f"http://{url_host}:{port}/portal/index.html"


def _export_outputs(
    root: Path, config: ProjectConfig, graph: ExtractedGraph, *, dry_run: bool
) -> None:
...
def _doctor_checks(config: ProjectConfig, *, strict: bool) -> dict[str, bool]:
    ...
    rag_configuration = not config.rag.enabled or bool(
    ...
    rag_dependencies = not config.rag.enabled or all(
        find_spec(module) is not None for module in ("fastapi", "neo4j_graphrag", "uvicorn")
    )
    checks = {
        ...
        "raw sources": any((root / config.graphify.input_path).rglob("*")),
        ...
        "GraphRAG configuration": rag_configuration,
        "GraphRAG dependencies": rag_dependencies,
    }
